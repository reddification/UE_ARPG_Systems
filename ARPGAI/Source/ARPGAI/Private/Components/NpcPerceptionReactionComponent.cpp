...
void UNpcPerceptionReactionComponent::BeginPlay()
{
	...
	if (const FNpcDTR* NpcDTR = NpcDTRH.GetRow<FNpcDTR>(""))
	{
		...
		if (!NpcDTR->NpcPerceptionReactionEvaluators.IsEmpty())
		{
			for (const auto* PerceptionReactionEvaluators : NpcDTR->NpcPerceptionReactionEvaluators)
				StoreReactionBehaviorEvaluators(PerceptionReactionEvaluators);
		}
	}
	...
	if (ensure(OwnerPawn))
	{
		auto AIController = Cast<AAIController>(OwnerPawn->GetController());
		if (AIController)
	...
	EReactionBehaviorType ReactionBehaviorType)
{
	...
	{
		auto GameMode = Cast<INpcSystemGameMode>(GetWorld()->GetAuthGameMode());
		const FTimespan& DateTime = GameMode->GetARPGAIGameTime();
...
void UNpcPerceptionReactionComponent::AddPerceptionReactionEvaluator(const UNpcPerceptionReactionEvaluatorsDataAsset* ReactionBehaviorEvaluatorsDataAsset)
{
	StoreReactionBehaviorEvaluators(ReactionBehaviorEvaluatorsDataAsset);
}

void UNpcPerceptionReactionComponent::StoreReactionBehaviorEvaluators(const UNpcPerceptionReactionEvaluatorsDataAsset* ReactionBehaviorEvaluatorsDataAsset)
{
	auto NpcGameMode = Cast<INpcSystemGameMode>(GetWorld()->GetAuthGameMode());
	...
	for (const auto& PerceptionReactionEvaluator : ReactionBehaviorEvaluatorsDataAsset->PerceptionReactionEvaluators)
	{
...
void UNpcPerceptionReactionComponent::RemovePerceptionReactionEvaluator(const UNpcPerceptionReactionEvaluatorsDataAsset* ReactionBehaviorEvaluatorsDataAsset)
{
	bool bRemoved = false;
	for (const auto* PerceptionEvaluator : ReactionBehaviorEvaluatorsDataAsset->PerceptionReactionEvaluators)
	{
